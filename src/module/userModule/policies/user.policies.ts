  ...
  async validateDeleteAgentRequest(req: any, res: any, next: any) {
    try {
      ...
      if (checkAssignment) {
        throw createError(
          400,
          'To delete this agent, please ensure it is not currently mapped with any machines.'
        );
      }
      next();
    } catch (err) {
  ...
  async validateUpdateSubRoleRequest(req: any, res: any, next: any) {
    try {
      ...
      for (const permission of permissions) {
        ...
        await validateUserApis.isModuleExist(permission.moduleId);
  ...
  async validateDeleteRoleRequest(req: any, res: any, next: any) {
    try {
      ...
      if (userCount > 0) {
        throw createError(
          400,
          templateConstants.ROLE_ALREADY_MAPPED(role?.name)
        );
      }
      res.locals.request = {
        subRole: role,
