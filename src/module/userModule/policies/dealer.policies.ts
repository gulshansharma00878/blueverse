    ...
    try {
      ...
      if (isUserExist) {
        ...
        if (isUserExist.phone === phone) {
          ...
          throw createError(400, templateConstants.ALREADY_EXIST(errorMessage));
        } else if (isUserExist.email === email) errorMessage = 'email';
        else
          errorMessage =
  ...
  async validateUpdateDealerRequest(req: any, res: any, next: any) {
    try {
      ...
      if (phone || email || pan_no) {
        ...
        if (dealer) {
          let errorMessage = null;
          if (dealer.phone === phone) errorMessage = 'phone';
          else if (dealer.email === email) errorMessage = 'email';
          else errorMessage = 'Pan Number';
          throw createError(400, templateConstants.ALREADY_EXIST(errorMessage));
        }
      }
