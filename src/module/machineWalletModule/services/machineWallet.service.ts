...
class MachineWalletService {
  async getMachineTransactionHistory(user: any, body: any) {
    try {
      ...
      const orderCondition = [['createdAt', sortOrder]];
  ...
  async getDealerMachineBalance(user: any, machineIds: any) {
    try {
      const machines: any = await Machine.findAndCountAll({
        attributes: [
          'machineGuid',
          'walletBalance',
          'blueverseCredit',
      ...
      let walleteBalance: any = 0.0;
      let blueverseCredit: any = 0.0;
      machines.rows.forEach((element: any) => {
        walleteBalance +=
          Number(element.walletBalance) + Number(element.topUpBalance);
        blueverseCredit += Number(element.blueverseCredit);
      });
      walleteBalance = walleteBalance.toFixed(2);
      blueverseCredit = blueverseCredit.toFixed(2);
