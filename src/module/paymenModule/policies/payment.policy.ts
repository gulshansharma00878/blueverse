...
class PaymentPolicy {
  async validateGenerateHashRequest(req: any, res: any, next: any) {
    try {
      const { userId, parentUserId, role } = res.user;
      ...
      if (
        !config.payUPaymentGatewayCredentials.productInfoArr.includes(
          type.toUpperCase()
        )
      ) {
      ...
      if (
        type === config.payUPaymentGatewayCredentials.productInfoObject.WALLET
      ) {
        /**
        ...
        const machineMemo = await MachineMemo.findOne({
          ...
          attributes: ['machineMemoId', 'totalAmount'],
        });
      ...
      if (
        ...
        if (process.env.NODE_ENV !== 'development') {
          ...
          if (machineWallet) {
            throw createError(404, 'Wallet recharge already for this month');
          }
        }
      }
