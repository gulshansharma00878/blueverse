...
class ExternalRoutes {
  ...
  registerRoutes() {
    ...
    this.externalRouter.post('/machine', externalController.createMachine);
    ...
    // It will generate all types of memo
    this.externalRouter.post(
      '/invoices',
      verifyClient.bind(verifyClient),
      externalController.generateInvoices
    );
    // Generate advance memo
    this.externalRouter.post(
      '/generate/advanceMemos',
      verifyClient.bind(verifyClient),
      externalController.generateAdvanceMemos
    );
    // Generate tax and blueverse credit memo
    this.externalRouter.post(
      '/generate/taxAndCreditMemos',
      verifyClient.bind(verifyClient),
      externalController.generateTaxAndCreditMemos
    );
    this.externalRouter.post(
      '/sync/machine/health',
      verifyClient.bind(verifyClient),
      externalController.syncMachineHealth
    );
    this.externalRouter.post(
      '/commencementAdvanceMemo',
      verifyClient.bind(verifyClient),
      externalController.generateCommencementAdvanceMemo
    );

    this.externalRouter.get('/washStats', externalController.getWashStats);

    // API to generate generateLowBalanceMachineNotification
    this.externalRouter.post(
      '/generateLowBalanceMachineNotification',
      verifyClient.bind(verifyClient),
      machineService.generateLowBalanceMachineNotifications
    );

    // Api to verify otp of booking
    this.externalRouter.post(
      '/verifyBookingOtp',
      externalController.verifyCustomerBookingOtp
    );
    ...
    this.externalRouter.post(
      '/generate/taxMemos',
      verifyClient.bind(verifyClient),
      externalController.generateTaxMemos
    );

    // Generate tax and blueverse credit memo
    this.externalRouter.post(
      '/generate/oldttaxAndCreditMemos',
      verifyClient.bind(verifyClient),
      externalController.generateOldTaxAndCreditMemos
    );
  }
}
