...
class FeedbackService {
  ...
  async validateFormDelete(req: any, res: any, next: NextFunction) {
    try {
      ...
      if (chk) {
        throw createError(
          409,
          'To delete this form, please ensure that it is not currently mapped with machines.'
        );
      }
      next();
    } catch (err) {
    ...
    try {
      ...
      if (merchantIds) {
        if (merchantIds.length === 0) {
          throw createError(
            409,
            templateConstants.PARAMETER_MISSING('Merchant Id')
          );
        }
      } else {
        ...
        const dealerCount = await User.count({
          where: {
            ...
            oemId: oemId,
            // isKycDone: true, //if dealer KYC is done ,then only his machine can map to agent
          },
        });
  ...
  async validateGetCustomerFeedbackForm(
    ...
    try {
      ...
      console.log(
        '🚀 ~ FeedbackService ~ skuNumber:',
        skuNumber,
        isSkuExist.QRGenerated
      );

      if (isSkuExist.QRGenerated == false) {
        throw createError(400, CONSTANT.SKU_NUMBER_QR_NOT_EXIST);
      }
  ...
  // if machine remove or unassigned then remove area and dealer details
  // async updateFormDetail(formId: string) {
  //   try {
  //     const machineCount = await Machine.count({
  //       where: {
  ...
  //       );
  //       console.log(
  //         '🚀 ~ file: feedback.service.ts:462 ~ FeedbackService ~ updateFormDetail ~ updateForm:',
  //         updateForm
  //       );
  //       const deleteFormDealer = await FormDealer.destroy({
  //         where: {
  //           formId: formId,
  //         },
  //       });
  //       console.log(
  //         '🚀 ~ file: feedback.service.ts:468 ~ FeedbackService ~ updateFormDetail ~ deleteFormDealer:',
  //         deleteFormDealer
  //       );
  //     }
  //     return;
