...
class UserRoutes {
  ...
  registerRoutes() {
    ...
    this.userRouter.get(
      '/dealer/list',
      verifyClient.bind(verifyClient),
      validateDealerApis.validateDealerListInput,
    ...
    this.userRouter.get(
      '/top/dealership',
     verifyClient.bind(verifyClient),
      dealerController.getTopDealerShip
    );
    ...
    this.userRouter.post(
      '/addDevice',
      verifyClient.bind(verifyClient),
      validateUserApis.validateTokenExist,
      userController.addDeviceToken
    );

    // fetch all token with user
    this.userRouter.get(
      '/userDeviceTokens',
      verifyClient.bind(verifyClient),
      userController.getUserDeviceTokens
    );

    // delete all token with user
    this.userRouter.delete(
      '/userDeviceTokens',
      verifyClient.bind(verifyClient),
      userController.deleteDeviceToken
    );

    // remove user only for development
    // this.userRouter.delete(
    //   '/permanentDelete',
    //   verifyClient.bind(verifyClient),
    //   validateUserApis.validateAdmin,
    ...
    // Login Response through Bearer token
    this.userRouter.post(
      '/authenticateToken/detail',
      verifyClient.bind(verifyClient),
      userController.getLoginResponse
    );
  }
}
