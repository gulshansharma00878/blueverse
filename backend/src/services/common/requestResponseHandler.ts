...
const verifyClient = async (req: any, res: any, next: any) => {
  try {
    ...
    if (!utility.isNullOrUndefined(headerAuth)) {
      auth = headerAuth;
    }
    const bodyAuth = req.body.authorization;
    if (!utility.isNullOrUndefined(bodyAuth)) {
      auth = bodyAuth;
    }
    if (!auth) {
      throw createError(
...
// This middleware verifies client
const authCustomerGuard = async (req: any, res: any, next: any) => {
  try {
    res.locals.requestIdentifier = utility.getTimeInMilliSeconds();
    ...
    // Verify the extracted token
    let userData;
    userData = await jwtService.verifyToken(auth);
    if (!userData) {
      throw createError(
        400,
        stringConstants.genericMessage.USER_NOT_AUTHORIZED
    ...
    const user = await Customer.findOne({
      where: { customerId: userData.userId },
      raw: true,
    });
